...
function WithdrawalsAdmin() {
  ...
  async function approve(id: string) {
    ...
    toast.success("Aprovado"); qc.invalidateQueries({ queryKey: ["admin-withdrawals"] });
  }
  async function reject(id: string) {
    const { error } = await supabase.rpc("reject_withdrawal", { p_id: id });
    if (error) { toast.error(error.message); return; }
    toast.success("Recusado"); qc.invalidateQueries({ queryKey: ["admin-withdrawals"] });
  }

  return (
    <Card className="bg-card">
      <CardHeader className="pb-2"><CardTitle className="text-sm">Pedidos de saque</CardTitle></CardHeader>
      <CardContent>
            ...
            {list.data?.map((d: any) => (
              <div key={d.id} className="py-3 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-muted-foreground">Tel: {d.profiles?.phone} · PIX: {d.pix_key}</div>
